...
public class ReminderUtilities {
    ...
    private static final long INTERVAL_STAMP = 1000 * 60 * 2;
    ...
    public static Map<String,Long> mBDevices = new HashMap<>();

    public static void addDevice(BluetoothDevice bd){
        if(mBDevices.size() == 0)
            mBDevices.put(bd.getAddress(), System.currentTimeMillis());

        if(!mBDevices.containsKey(bd.getAddress())){
            mBDevices.put(bd.getAddress(), System.currentTimeMillis());
            System.out.println("added new device " + bd.getAddress());
        }
        else {
    ...
    public static void showStackDevices(){
        if(mBDevices.size() > 0){
            for (Map.Entry<String,Long> entry : mBDevices.entrySet())
                System.out.println("Key = " + entry.getKey() +
                        ", Value = " + entry.getValue());
        }
    }

    public static void checkIntervalValidity(BluetoothDevice bl){
        Long currentStamp = System.currentTimeMillis();
        Long startStamp = mBDevices.get(bl.getAddress());
        if((currentStamp - startStamp) > INTERVAL_STAMP){
            //save both devices in data base with one location
        }
    }
